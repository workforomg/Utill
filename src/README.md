...
| 파일 | 전역 객체 | 용도 |
|---|---|---|
| `dist/crack.js` | `Crack` | Core 전체 |
| `dist/crack-ui.js` | `CrackUI` | UI 도구 전체 |
| `dist/crack-discovery.js` | `CrackDiscovery` | 검색·탐색 |
| `dist/crack-profiles.js` | `CrackProfiles` | 프로필·설정 |
| `dist/crack-community.js` | `CrackCommunity` | 공지·알림·피드 조회 |
| `dist/crack-library.js` | `CrackLibrary` | 작품·채팅 목록 |

각 번들은 단독으로 로드할 수 있습니다. 예를 들어 공지만 조회하려면 `crack-community.js`를 로드한 뒤 `CrackCommunity.createCommunityAPI()`를 사용합니다.
